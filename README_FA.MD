...
| نوع رمزنگاری | TLS 1.3 (در QUIC) | Noise (Curve25519) | AES-GCM به‌صورت پیش‌فرض در پروفایل سخت‌سازی‌شده (حالت‌های قدیمی بدون احراز اصالت در config رد می‌شوند) |
...
| `DATA_ENCRYPTION_METHOD` | `5` | `3=AES-128-GCM`، `4=AES-192-GCM`، `5=AES-256-GCM` | روش رمزنگاری payloadهای تونل است و باید با سرور یکسان باشد.<br>در پروفایل سخت‌سازی‌شده، حالت‌های بدون احراز اصالت (`0=None`، `1=XOR`، `2=ChaCha20`) رد می‌شوند. |
...
| پارامتر | مقدار نمونه | مقادیر مجاز / رفتار واقعی | توضیح کامل |
| :--- | :--- | :--- | :--- |
| `DATA_ENCRYPTION_METHOD` | `5` | `3..5` | باید با کلاینت یکی باشد.<br>پروفایل سخت‌سازی‌شده مقدار نامعتبر یا بدون احراز اصالت را به‌جای بازگشت به XOR رد می‌کند. |
